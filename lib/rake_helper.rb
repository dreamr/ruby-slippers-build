...
module SlipperEnv
  ...
  %w(build release).each do |type|
    define_method "log_#{type}" do |msg|
      File.open(ENGINE_ROOT+"/log/#{type}.log", "w") {|file| file.puts msg}
    end
  end

  def bad_return(msg)
    print red, bold, "Yeah, so that didn't work. #{msg}", reset, "\n"
    exit
  end
  
  def release_version
    path=ENGINE_ROOT+"/log/release.log"
    File.open(path).read.scan(/([0-9]+)\.([0-9]+)\.([0-9]+)/) do |r,p,b|
      "#{r}.#{p}.#{b}"
    end
  end
  
  def build_version
    path=ENGINE_ROOT+"/VERSION"
    File.open(path).read.scan(/([0-9]+)\.([0-9]+)\.([0-9]+)/) do |r,p,b|
      "#{r}.#{p}.#{b}"
    end
  end
  ...
  def increment_version(type, current_version)
    ...
    timestamp = lambda {|now| now.strftime("on %m/%d/%Y at %H:%m") }.call(DateTime.now)
